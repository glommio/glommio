...
pub(crate) mod test {
    ...
    dma_file_test!(file_fallocate_alocatee, path, _kind, {
        ...
        new_file
            ...
            .await
            .expect("fallocate failed; should be noop");
                ...
                local_ex.run(async move {
                    ...
                    if let GlommioError::CanNotBeClosed(ResourceType::File(_), _) =
                        cloned_file.close().await.expect_err(
                            "error should be returned by close if a clone exists & that includes clones materialized on other threads (see close_fails_if_clone_exists)",
                        )
                    {
                    } else {
