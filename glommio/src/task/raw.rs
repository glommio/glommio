    ...
    S: Fn(Task),
{
    ...
    /// An acquire load of one reference proves exclusive ownership because no
    /// weak references can recreate ownership, allowing direct destruction.
    unsafe fn drop_waker(ptr: *const ()) {
        ...
        loop {
            ...
            if refs == 2 && (*header).active.load(Ordering::Acquire) {
                if Self::thread_id() == Some((*header).executor_id) {
                    defer!(Self::release(ptr));
                    Self::do_cleanup(ptr);
                    return;
                }
                if let Some(notifier) = Self::from_ptr(ptr)
                    .notifier()
                    .filter(|notifier| notifier.accepts_foreign_wakes())
                {
                    notifier.queue_waker(
