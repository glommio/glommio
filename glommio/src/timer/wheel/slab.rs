...
//
//! Stable storage for wheel entries.
//!
...
impl<T> Slab<T> {
    ...
    pub(crate) fn remove(&mut self, id: Key) -> Option<T> {
    ...
    pub(crate) fn get(&self, id: Key) -> Option<&T> {
    ...
    pub(crate) fn get_mut(&mut self, id: Key) -> Option<&mut T> {
    ...
    pub(crate) fn id_at(&self, slot: SlotIndex) -> Option<Key> {
        match self.slots.get(slot.as_usize())? {
            Slot::Occupied { generation, .. } => Some(Key {
                slot,
                generation: *generation,
            }),
            Slot::Vacant { .. } => None,
